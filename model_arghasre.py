...
def validate(model, val_loader, loss_fn, org_weights, rank, epoch=None, num_epochs=None):
    model.eval()
    with torch.no_grad():
        val_loss = 0
        val_cai = 0
    ...
    print(f"Validation Loss: {avg_val_loss:.4f}, Validation CAI: {avg_val_cai:.4f}, Validation CAI GT: {avg_val_cai_gt:.4f}")
